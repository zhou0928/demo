<template>
  <div>
    ...
    <el-card>
      ...
      <!--      选择商品分类区域-->
      <el-row>
        <el-col :span="4" class="cols">选择商品分类：</el-col>
        <el-col :span="20">
          <el-cascader
        ...
        <el-tab-pane label="动态参数" name="many">
          ...
          <el-table
            ...
            <el-table-column
              ...
              width="300">
              <template slot-scope="scope">
                <el-button type="primary" icon="el-icon-edit" size="small" @click="showEditDialog(scope.row.attr_id)">编辑
                </el-button>
                <el-button type="danger" icon="el-icon-delete" size="small" @click="deleteParams(scope.row.attr_id)">删除
                </el-button>
              </template>
            </el-table-column>
          </el-table>
...
export default {
  ...
  methods: {
    ...
    async showEditDialog(id) {
      // console.log(id)
      // 查询当前参数的信息
      const {data: res} = await axios.get(`categories/${this.cateId}/attributes/${id}`, {params: {attr_sel: this.activeName}})
      if (res.meta.status !== 200) {
...
// 根据Id删除对应的一行数据
    async deleteParams(id) {
      ...
      if (confirmResult !== 'confirm') {
        ...
        // console.log(confirmResult, id)
        const {data: res} = await axios.delete(`categories/${this.cateId}/attributes/${id}`)
        if (res.meta.status !== 200) {
